...
if df_sales is not None and st.session_state['page'] != "NLP":
    ...
    # Página: Vendas Regionais
        ...
        if st.session_state['chart_type'] == 'Distribuição de Vendas por Região':
            ...
            fig8 = px.imshow(product_mix, labels=dict(x='Vehicle Model', y='Dealer Region', color='Sales Count'),
                             title='Product Mix Heatmap')
            st.plotly_chart(fig8)

# Página: NLP
elif df_nlp is not None and st.session_state['page'] == "NLP":
    st.title('Dashboard NLP')

    # Inicializar o estado da sessão para o tipo de gráfico se ainda não foi definido
    if 'chart_type' not in st.session_state:
        st.session_state['chart_type'] = 'Análise de Sentimentos'
    ...
    # Exibir o gráfico com base na escolha do botão
    if st.session_state['chart_type'] == 'Análise de Sentimentos':
        st.write("Sentiment Analysis Placeholder")

    elif st.session_state['chart_type'] == 'Word Cloud':
        st.write("Word Cloud Placeholder")

    elif st.session_state['chart_type'] == 'Bigramas':
        # Gerar bigramas
        vectorizer = CountVectorizer(ngram_range=(2, 2))
        X = vectorizer.fit_transform(df_nlp['review'].fillna(''))
        bigrams = pd.DataFrame(X.toarray(), columns=vectorizer.get_feature_names_out())
        bigram_counts = bigrams.sum().sort_values(ascending=False)
        
        fig_bigramas, ax_bigramas = plt.subplots(figsize=(12, 8))
        bigram_counts[:20].plot(kind='barh', color='skyblue', ax=ax_bigramas)
        ax_bigramas.set_title('Top 20 Bigrams')
        ax_bigramas.set_xlabel('Frequency')
        st.pyplot(fig_bigramas)
